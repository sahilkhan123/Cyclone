...
class Machine:
    ...
    def gate_time(self, sys_state, trap_id, ion1, ion2):
        assert ion1 != ion2
        mp = self.mparams
        #print("sys state", sys_state.trap_ions)
    ...
    def split_time(self, sys_state, trap_id, seg_id, ion1):
        #traps_dict = {}
        #for mach in self.traps:
            #traps_dict[mach.id] = mach
        t = self.traps[trap_id]
        #t = traps_dict[trap_id]
        split_estimate = 0
        split_swap_count = 0
        ion_swap_hops = 0
        split_swap_hops = 0
        i1 = 0
        i2 = 0
        #print("sys state", sys_state.print_state())
        #print("t orientation", t.orientation)
        #print("seg id", seg_id) #debug algorithm: for all segments in the machine, they should be in the connecting traps orientation
        #print("trap id", trap_id) #machine graph is saying due to path that segment is in its adjacency but traps orientation list is saying its not
        ...
        if ion1 == ion2:
            ...
            if mp.swap_type == "GateSwap":
                ...
                p2 = sys_state.trap_ions[trap_id].index(ion2)
                num_hops = abs(p1-p2)
                split_estimate = 1 #1 microseconds to rotate a whole chain is faster than 300 with merges/splits? something is off
                ion_swap_hops = num_hops
                """
            
            else:
                assert(0)
                print("Error, this is an unrecognized SWAP tyepe")
