...
base_error_rate  = float(sys.argv[1])
...
print("Variable baseline value", baseline) #should be called benchmark type instead of baseline
if (baseline== "Baseline"):
	print("Forwarding results to baseline file")
	fname = './CODE_' + str(n) + '_' + str(k) + '_' + str(d) + '/result_baseline'
elif (baseline=="GRIP"): 
	fname = './CODE_' + str(n) + '_' + str(k) + '_' + str(d) + '/result_GRIP'
else:
...
for trial in range(num_trials):
	...
	"""
	with open("singleCycleCircQASMobj.pkl", "wb") as file:
		pkl.dump(mydata["cycle"], file)
	exit()
	"""
...
logical_error_rate = bad_trials/num_trials

print(str(base_error_rate) + '\t' + str(error_rate) + '\t' + str(num_cycles) + '\t' + str(num_trials) + '\t' + str(bad_trials) + '\t' + str(logical_error_rate),file=open(fname,'a'))
