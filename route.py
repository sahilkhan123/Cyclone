...
class FreeTrapRoute:
    ...
    def find_route(self, source_trap, dest_trap):
        ...
        print("self.ss.trap_ions", self.ss.trap_ions)
        #traps_dict = {}
        #for mach in m.traps:
            #traps_dict[mach.id] = mach
        for k in list(self.ss.trap_ions.keys()):
            print("k", k)
            #temp = m.traps[k].capacity
            #other = ss.trap_ions[k]
            trap_free_space[k] = m.traps[k].capacity - len(ss.trap_ions[k])
            #trap_free_space[k] = traps_dict[k].capacity - len(ss.trap_ions[k])
        for u, v in m.graph.edges:
            if (type(u) == Trap and type(v) == Junction) or (type(u) == Trap and type(v) == Trap): #LEFT OFF HERE, SO DON'T NEED TO INCREASE CAPACITY FOR REBALANCING. 
                e0 = u
                e1 = v
