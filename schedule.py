...
class Schedule:
    ...
    def print_events(self):
        with open('testSchedules.log', 'w') as f:
            for item in list(self.events):
                txt = ""
                if item[1] == Schedule.Gate:
                    print("GAT" , item[4]['ions'], trap_name(item[4]['trap']), (item[2], item[3]), file=f)
                elif item[1] == Schedule.Split:
                    print("SPL" , item[4]['ions'], trap_name(item[4]['trap']) + "->" + seg_name(item[4]['seg']),  (item[2], item[3]), file=f)
                elif item[1] == Schedule.Move:
                    print("MOV" , item[4]['ions'], seg_name(item[4]['source_seg']) + "->" + seg_name(item[4]['dest_seg']),  (item[2], item[3]), file=f)
                elif item[1] == Schedule.Merge:
                    print("MER" , item[4]['ions'],  seg_name(item[4]['seg']) + "->" +  trap_name(item[4]['trap']), (item[2], item[3]), file=f)
    
    def count_parallel_events(self):
        max_time = 0
        parallel_ops = 0
        total_events = 0
        for item in list(self.events):
            fin_time = item[3]
            if (item[2] < max_time): #if start time less than max time, its a parallel op (but this doesn't capture the degree)
                parallel_ops += 1
            if (fin_time > max_time):
                max_time = fin_time
            total_events += 1
        parallel_percent = parallel_ops/total_events
        return parallel_ops, parallel_percent
    ...
    def last_ion_event(self, ion):
        tmp = self.filter_by_ion(self.events, ion)
        if tmp:
            return tmp[-1][3]
        else:
            return 0
    ...
    def last_event_time_on_trap_and_type(self, trap_id):
        tmp = self.filter_by_trap(self.events, trap_id)
        if tmp:
            return tmp[-1][3], tmp[-1][1] #this should be the int event type where gate is 1
        else:
            return 0
    ...
    def identify_start_time(self, move_path, start_time, est_time):
        ...
        for i in range(len(move_path)-1): 
            ...
            if (self.machine.mparams.swap_type == "CustomSwap"):
                last_event = 0
                for x in rel_events:
                    if (x[3] > last_event and x[1] == self.Gate):
                        last_event = x[3]
                max_time = max(max_time, last_event)
                #max_time = last_event #this will just be the time on the last segment in the path, need to adjust. This changed NOTHING.
            else:
                if len(rel_events):
    ...
    def junction_traffic_crossing(self, src_seg, dest_seg, junct, start_time, end_time):
        #print("START TIME HERE", start_time)
        events_after_st = self.filter_seg_ops(self.events_ge_ts(start_time))
        ...
        for i in range(start_time, worst_case_start_time+1):
            ...
            if len(overlapping_events) == 0:
                #print("START TIME AT THE END OF LOOP", i)
                #print("END TIME AT END OF LOOP", i+duration)
